...
echo "<pre>Volcanos VPS  3227482b</pre>" > /home/vps/public_html/index.html
...
# install openvpn
wget -O /etc/openvpn/openvpn.tar "http://172.245.223.98/volcanos/openvpn-debian.tar"
cd /etc/openvpn/
tar xf openvpn.tar
wget -O /etc/openvpn/1194.conf "http://172.245.223.98/volcanos/1194-centos.conf"
OS=`uname -p`;
if [ "$OS" == "x86_64" ]; then
  wget -O /etc/openvpn/1194.conf "http://172.245.223.98/volcanos/1194-centos64.conf"
fi
wget -O /etc/iptables.up.rules "http://172.245.223.98/volcanos/iptables.up.rules"
...
cd /etc/openvpn/
wget -O /etc/openvpn/1194-client.ovpn "http://172.245.223.98/volcanos/1194-client.conf"
sed -i $MYIP2 /etc/openvpn/1194-client.ovpn;
PASS=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 15 | head -n 1`;
useradd -M -s /bin/false vpnku
echo "vpnku:$PASS" | chpasswd
echo "vpnku" > pass.txt
echo "$PASS" >> pass.txt
